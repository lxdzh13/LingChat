...
class VoiceCheck:
    ...
    def prepare_vits_directory(vits_path: Path):
        ...
        if not vits_path.exists():
            ...
            assert vits_archive.exists(), (
                f"VITS语音合成器压缩包未找到，请手动下载到{vits_archive}。"
            )
        ...
        if not vits_model_path.exists():
            ...
            assert vits_model_archive.exists(), (
                f"VITS模型压缩包未找到，请手动下载到{vits_model_archive}。"
            )
    ...
    def main():
        ...
        vits_thread = threading.Thread(
            target=VoiceCheck.run_vits_process,
            args=(vits_ready_event, status_ok),
            daemon=True,
        )
