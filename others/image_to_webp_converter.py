...
def convert_to_webp(input_path, output_path=None, quality=80, lossless=False):
    ...
    try:
        with Image.open(input_path) as img:
            ...
            print(
                f"  文件大小: {input_size} bytes -> {output_size} bytes (压缩率: {compression_ratio:.1f}%)"
            )
            return output_path
...
def batch_convert(
    input_folder, output_folder=None, quality=80, lossless=False, supported_formats=None
):
    """
    批量转换文件夹中的所有图片
...
def main():
    ...
    parser.add_argument(
        "-q", "--quality", type=int, default=93, help="WebP质量 (0-100)，默认为93"
    )
    parser.add_argument(
        "--lossless",
        action="store_true",
