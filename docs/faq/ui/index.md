...
这是 pywebview 的 winforms 后端与嵌入式 Python 3.13 不兼容导致的。新版本已改为优先使用 EdgeChromium 后端，通常不会再遇到。

如果你使用的是 **Win10 LTSC**（长期服务版本）或其他精简系统，可能没有预装 WebView2 Runtime，需要手动安装：

- 下载地址：[Microsoft WebView2 Runtime](https://developer.microsoft.com/zh-cn/microsoft-edge/webview2/#download)
- 选择"常青独立安装程序"下载安装即可

安装后重启 LingChat 就能正常打开前端窗口了。如果仍然失败，程序会自动降级到浏览器模式，控制台会提示访问地址。
